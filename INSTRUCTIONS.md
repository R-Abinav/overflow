...
| Question | Decision |
|---|---|
| Model | Original plan: Claude API only. Actual: Gemini (`temperature: 0`) was primary through Phase 0-4 dev/testing for cost reasons; Claude became primary at Phase 5 for the live demo, `llm.ts` checks `ANTHROPIC_API_KEY` first with Gemini as fallback. See `docs/phase-2.1-results.md` for what was found/fixed switching providers. |
| ollama/tinyllama | Originally planned for the "not constrained" local fast path. **Superseded at Phase 1.5**: the unconstrained-requester path was found to be a mocked stub (never actually called Ollama) and was replaced with real `generateCandidateCode()` + `runScript()` — the same sandbox executor the provider uses. Ollama is not required to run this project. |
